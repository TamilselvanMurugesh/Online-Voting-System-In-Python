...
def count_reset():
    ...
    df=df[['voter_id','Name','Gender','Zone','City','Passw','AadharNo','hasVoted']]
...
def reset_voter_list():
    df = pd.DataFrame(columns=['voter_id','Name','Gender','Zone','City','Passw','AadharNo','hasVoted'])
    df=df[['voter_id','Name','Gender','Zone','City','Passw','AadharNo','hasVoted']]
    df.to_csv(path/'voterList.csv')
...
def taking_data_voter(name,gender,zone,city,passw,AadharNo):
    df=pd.read_csv(path/'voterList.csv')
    df=df[['voter_id','Name','Gender','Zone','City','Passw','AadharNo','hasVoted']]
    row,col=df.shape
    if row==0:
