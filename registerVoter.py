...
def reg_server(root,frame1,name,Gender,zone,city,passw,AadharNo):
    if(passw=='' or passw==' '):
        msg = Message(frame1, text="Error: Missing Fileds", width=500)
        msg.grid(row = 10, column = 0, columnspan = 5)
        return -1

    vid = df.taking_data_voter(name, Gender, zone, city, passw, AadharNo)
...
def Register(root,frame1):
    ...
    Label(frame1, text="AadharNo:   ", anchor="e", justify=LEFT).grid(row = 7,column = 0)
    ...
    e8 = Entry(frame1, textvariable = AadharNo).grid(row = 7, column = 2)
    ...
    reg = Button(frame1, text="Register", command = lambda: reg_server(root, frame1, name.get(), Gender.get(), zone.get(), city.get(), AadharNo.get(), password.get()), width=10)
